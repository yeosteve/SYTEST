...
class Bpmodel extends CI_Model {
    ...
    /**
     * sortTheBastard
     * when something is dropped on a target update the dragee's excel and category, and sortorder, which will have been taken from the elements in the list ???
     */
    function sortTheBastard()
    {
        return true;
        //if(isset($_GET['listItem'])){ //if($this->input->post('xxxx')){
        //$test='';
        //foreach ($_GET['listItem'] as $position => $item){
        //$sql = "UPDATE tbl_list SET position = '$position' WHERE id = '$item'";
        // run the query
...
/**
 * getAllBPs
 * $arrCourses = array(id=>'name',
 *                      category=>array(excel=>array(
 *                          
 *
 *                      ))
 *
 *
 *                      array('BP' => array(id, name)
 *    [BP[0]] = id,  BP[1] = name
 *
 *    array('excel'=>array(0=>'PASS', 1=>'BETTER')
 */
function allBPs() {
     $arrAll = array();
 ...
 */
function allCourses($courseID=0){
    $arrAll = array();
    ...
    //foreach($arrCourses as $coursekey => $rowCourse){
    //    $id = $rowCourse['id'];
    //    $arrAll[$id] = $rowCourse['name'];//[1] => AE01 - Web Communication
    //    $arrCats = $this->getCatsByCourse($id);
    //}
    
        // print_r($arrAll);
        // set the module name as the key of the $arrAll
        // we will assign the value of the array of categories for this module in the first step,
        //foreach($arrAll[$rowCourse] as $key => $rowCourse){
        ////
        //$arr = $this->getCatsByCourse($arrAll[$key]['id']);
        //foreach($arr as $k=>$arrCats){
        //    $id = $arrCats['id'];
        //   $arrAll[$k]['cats'][$id] =  $arrCats['name'];// the course names
        //}
      
       //for($i=0;$i<=1;$i++){
       // $arrAll[$key]['cats']['excel'][$i] = $this-getBPbyCat($arrAll[$key]['cats']['id'] , $i);
       //}
       
       
       
    //    
    //    $arrAll[$rowCourse['id']]['courseName'] = $arrAll[$rowCourse['name']];
        //$arrCats = $this->getCatsById($rowModule['id']); // get array of categories
        //    foreach($arrCats as $id => $rowCats){
        //        $bpPass = $this->getBPbyCat($rowCats['id'], 0);
        //        $bpDistinction = $this->getBPbyCat($rowCats['id'], 1);
